...
from config import DEFAULT_PROMPT, load_config
...
def process_file(code_file, llm_model, output_excel, export_path=None):
    """
    Process a single file for bloat removal.
    
    Args:
        code_file: Path to the Python file with bloated code
        llm_model: LLM model to use for code optimization
        output_excel: Path to output Excel file
        export_path: Path to export the optimized code (if None, original file is updated)
    """
    try:
        ...
        optimized_code = llm_handler.optimize_code(llm_model, original_code, DEFAULT_PROMPT)
        optimized_loc = code_processor.count_lines_of_code(optimized_code)
        logger.info(f"Optimized code has {optimized_loc} lines")
        
        # Step 3: Save optimized code (to export path or original file)
        ...
        # Check if the optimized code is significantly different from the original
...
def main():
    ...
    parser.add_argument('--code_file', type=str, required=True, help='Path to the Python file with bloated code')
    ...
    parser.add_argument('--custom_prompt', type=str, help='Custom prompt to use instead of the default')
    parser.add_argument('--export_path', type=str, help='Path to export the optimized code (if not specified, original file is updated)')
    ...
    if args.custom_prompt:
        DEFAULT_PROMPT = args.custom_prompt
    ...
    # Process the file
    success = process_file(code_file, args.llm_model, args.output_excel, export_path)
